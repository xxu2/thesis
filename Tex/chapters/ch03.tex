...
thus need to develop an retrieval algorithm that ueses statistically
optimized multi-variable fitting, in which the solution is sought not
only rely on the preassumed classes of potential solutions, but also in
a continuous space of slotions under statistically formulated criteria
optimizing the error distributon of the retrieval parameters. In this
chapter, I first present the general theory of inverse problem (section
\ref{sec:invtheory}), covering the Bayesion-based inversion (section
...
considered constraining its \textit{a pripri} and smoothness feature 
...
conrtol of measurements (section \ref{subsec:alg-qaulity}).

\section{Inversion Theories} \label{sec:invtheory}

\subsection{Maximum a posteriori solution of an inverse problem} \label{subsec:map}
